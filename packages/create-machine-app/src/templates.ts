...
export const TEMPLATES: readonly TemplateDescriptor[] = [
  ...
  },
  {
    ...
    description:
      'Electron desktop app: multi-format model picker (.gguf via vendored llama-server subprocess, .task via @mediapipe/tasks-genai in renderer, .litertlm recognized) + machineai-activation-ui hooks. Self-updating llama.cpp via scripts/fetch-llama-cpp.js.',
    target: 'electron',
    nextSteps: [
      '{pm} install',
      '{pm} run fetch:llama',
