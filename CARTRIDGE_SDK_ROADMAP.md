...
- **Session 17 (2026-07-25) made the adapter layer real.** The llama-server adapter existed in **four** divergent hand-written copies across this repo and the apps consuming it; it is now one implementation in `src/runtime/` (portable `llamaServerRuntime` + `stubRuntime` from the main barrel, `ensureLlamaServer`/`startLlamaServer`/`discoverLlamaServer` from `machineai-activation/node`). `machine serve` exposes a local model over OpenAI-shaped HTTP so a **non-JS** app can use one at all — that was a hard ceiling before, not a gap. All three target apps (`Collecta-Local`, `Ingredient analyzer`, `Agent On Deck - Local`) now run on the live SDK; see `TODO.md` §1c and §2b, and `PORTING.md`. Two defects were found by running a real model that 199 passing tests had not: thinking models reported as generating **0 tokens** (`reasoning_content` was ignored), and a still-loading model killed as a timeout.
- **Product focus (2026-07-24):** Machine AI is **the adapter layer for local models** — plug a GGUF into any app, on any OS, to **test** it and to **package** it. Scope is Windows + macOS + Linux + Android + iOS. The Vercel-AI-SDK-shaped surface is the **porting on-ramp, not the identity**; the activation contract is the local-first vocabulary and should lead. Full statement in `TODO.md` §0. Judge new work by: *does this make "plug a GGUF into any app on any OS" easier?*
- **Read these first:** `TODO.md` (focus + prioritized queue), `PLATFORM_MATRIX.md` (honest per-platform ledger), `sdkgaps.md` (defects found in a real integration).
- **Repo:** `MachineActivationSDK` — npm workspace root (`"workspaces": ["packages/*"]`); the SDK itself still lives at the repo root. All four packages are published to public npm at `0.2.0-beta.1` (verified 2026-07-24), but **no git tag exists** for that release — see `TODO.md` §9 before tagging, the release workflow fires on `activation-sdk-v*`.
- **Tests:** root `npm run check` (SDK: 199 tests) + `check:ui` (14) + `check:scaffolder` (52, includes `typecheck:adapters`) + `check:capacitor`, aggregated by `check:all`.
- **Last verified:** 2026-07-24 — SDK 199/199, UI 14/14, scaffolder 52/52, `check:all` clean. **The suite now takes ~37 s, not ~4 minutes.**
- **The "pre-existing CLI pull/search localhost flake" is FIXED, and it was never a flake.** One test (`machine pull exits 1 when the catalog does not contain the id`) used the blocking `runCli`/`spawnSync` while needing the in-process catalog server to answer, so it deadlocked until undici's 300 s timeout and then passed for the wrong reason — 309 s of every run. All three harnesses are now sequential with a per-test timeout, which is what made it visible. See `TODO.md` §8.
- **Windows x64 is verified on real hardware** (first real inference in the project, 2026-07-24). macOS/Linux/iOS/Android are still wired-only. `machine doctor --run <model.gguf>` is the one-command way to do each remaining platform.
- **Agent entry point:** `AGENTS.md` + `machine describe` (dumps the full SDK surface as JSON in one call; includes a `ui` section listing `machineai-activation-ui` hooks + components, a `scaffolder` section listing `create-machineai-app` templates, and `doctor` in the CLI section).
- **Fastest way to see the thing work:** `machine doctor <model.gguf>` — static, offline, no catalog. Add `--run` (with `MACHINE_LLAMA_SERVER` set, or a `vendor/llama-cpp/<slug>/` binary nearby) to load the model and measure it.
...
| **M6** — `machineai-activation-ui` headless UI kit | ✅ shipped | First workspace split: `packages/ui/` ships `machineai-activation-ui` (version `0.1.0-alpha.1`) with three subpath entry points (`machineai-activation-ui`, `machineai-activation-ui/web`, `machineai-activation-ui/native`). Core hooks: `MachineProvider`, `useMachineContext`, `useMachineModel`, `useActivationSnapshot`, `useInference`, `useCartridgeFilter`, plus `formatBytes` / `formatTokensPerSecond`. Five components per target (web: DOM elements; native: RN primitives via an ambient `react-native.d.ts` stub so no heavyweight devDep). 14 pure-logic unit tests via the SDK harness; runtime component tests deferred to M8. Root gains `typecheck:ui`/`test:ui`/`build:ui`/`check:ui`/`check:all` — root `check` stays SDK-only so the shipped baseline never drifts. |
| **M5** — `create-machine-app` scaffolder + 5 templates | ✅ shipped | Second workspace split: `packages/create-machine-app/` ships `create-machineai-app@0.1.0-alpha.1` — a zero-runtime-dep CLI (Node built-in `readline` for prompts, hand-rolled ~60-LOC arg parser) with five templates: `node-script` (Node + `createMachine` + `generateText` + `createNodeCartridgeResolver`), `expo-local-chat` (Expo RN + `llama.rn` + `machineai-activation-ui/native`), `rn-cli-local-chat` (bare RN + `llama.rn` + `machineai-activation-ui/native`), `next-local-chat` (Next.js 14 app-router + `@mlc-ai/web-llm` wired in-template + `machineai-activation-ui/web`), and `electron-local-chat` (Electron + `node-llama-cpp` in main process + IPC-backed renderer runtime + `machineai-activation-ui/web`). Placeholder substitution via `.tmpl` suffix convention (`{{APP_NAME}}`, `{{PACKAGE_MANAGER}}`). `TemplateTarget` union now `'node' \| 'expo' \| 'react-native' \| 'next' \| 'electron'`. Session A shipped scaffolder core + `node-script` + `expo-local-chat` (session 8); Session B added the remaining three (session 9). Full test matrix: ~36 tests across all five templates covering tree shape, placeholder substitution, wiring assertions, `.tmpl` leakage, exit codes. Templates intentionally not typechecked in the SDK repo. |

### What to work on NEXT

**→ Run a model on macOS, Linux, iOS and Android.** `TODO.md` §2. Windows is now verified end-to-end; the other four are what the five-platform claim still rests on. This is a much smaller job than it was: on macOS/Linux it should be `npm run fetch:llama` in the electron template, then `machine doctor --run model.gguf --gpu-layers 999`, and read the report. Mobile needs a host app because `llama.rn` can't be driven from a CLI.
...
**Then, in order:** grow the catalog past one cartridge (`TODO.md` §3 — the catalog is live and `machine pull` works, but a catalog of one is a demo; a 1–3B instruct model and a different architecture would make `machine search` do real work); register the PyPI trusted publisher so `pip install machine-activation` works (`TODO.md` §9 — the package builds and the workflow is wired, but only the account owner can do the registration); wire `local_model.py` into Agent On Deck's call sites (7 of 13 implemented, 0 routed); housekeeping in §9 (git tag, `.pack/` cleanup).

`streamText({ tools })` is **done** as of session 18 — see `TODO.md` §1d. It was the largest remaining API gap.
...
M8 is ✅ shipped. Reference App 01 (Ingredient Analyzer port) shipped sessions 10/12; Reference App 02 (Second Brain — local conversational notebook on Electron) shipped sessions 13–14. Reference App 02 lives at `Second Brain - Activation SDK\`.
...
- ~~**Publish to npm.**~~ ✅ Done 2026-06-08 — all four workspaces are live at `0.2.0-beta.1` (verified against the registry 2026-07-24). Several docs in this repo claimed otherwise until 2026-07-20. Still open: **no git tag** links the published artifact to a commit — read `TODO.md` §9 before tagging, since `.github/workflows/activation-sdk-release.yml` fires `npm publish` on any `activation-sdk-v*` tag and republishing an existing version fails with `EPUBLISHCONFLICT`.
- **Drop the `file:` deps in the reference apps** and replace with version ranges. `Collecta-Local` still pins a vendored tarball predating both the rename and the publish (`TODO.md` §9).
- **Migration guide.** "Coming from Vercel AI SDK / OpenAI / Anthropic — swap one import." Reference both apps as proof. Frame it as the **porting on-ramp**, consistent with the 2026-07-24 focus — not as the product's identity.
- **Docs site.** API reference + getting-started + reference-app walkthroughs. Lead with the activation contract and the cartridge/catalog story; the AI-SDK-shaped surface is the migration path, not the headline.

**M7.5 (web runtime adapter) stays open** but is no longer M8-blocking. Session 9 sidestepped it by wiring `@mlc-ai/web-llm` at the template level (inside `next-local-chat/src/lib/webLlmRuntime.ts`) rather than as a new `machineai-web-llm-runtime` workspace — same call-site ergonomics, no new SDK surface to maintain. Revisit M7.5 only if a future app wants to import a WebLLM runtime at SDK granularity rather than copying the template file.

Deferred (do NOT pull into M8):
- **Component runtime tests for `machineai-activation-ui`.** Waiting for M8 when a real app exercises them end-to-end. TS typecheck + pure-logic unit tests are the current safety net.
...
- **Stronger rule (session 12):** Node-only helpers must NOT be re-exported from the main barrel `src/index.ts` or from the `cartridge/index.ts` / `catalog/index.ts` sub-barrels. Static `export * from './nodeFs'` keeps `node:fs` in a Vite/Rollup browser bundle's dependency graph regardless of `sideEffects: false` and the `browser` field. Node-only re-exports live in `src/node.ts` and ship via the `machineai-activation/node` sub-export. Apps and CLI commands that need them either import the sub-export or deep-import the underlying file (e.g. `import { unpackCartridge } from 'machineai-activation/cartridge/nodeUnpackCartridge'` in CLI commands, which compile to `../../cartridge/nodeUnpackCartridge`).
...
- **Tests use a simple harness** (`tests/_harness.ts`, plus copies in `packages/ui/tests/` and `packages/create-machine-app/tests/`). No Jest/Mocha — just `test(name, fn)` + `finish()`. Register new test files in `tests/run.ts`. **Since session 16 the harness runs tests sequentially in registration order, each under a timeout** (`MACHINE_TEST_TIMEOUT_MS`, default 60 s), and annotates anything slower than 5 s with its duration. If you add a CLI test that needs the in-process catalog server, you MUST use `runCliAsync` — `runCli` uses `spawnSync`, which blocks the event loop the server runs on and deadlocks. That mistake cost this repo a 309-second "flake" for months.
- **The GGUF header parser is portable** (`src/model/gguf.ts` — no `node:*`, operates on a `Uint8Array`, signals `GgufTruncatedError` when handed too short a prefix). `src/model/nodeGguf.ts` does the file reading and grows its read window automatically. Don't merge them; RN/browser consumers can parse a header from bytes they obtained however they like.
- **`machine doctor`'s live runtime is deliberately not exported.** `src/bin/commands/doctorRuntime.ts` implements `ActivationRuntime` over a `llama-server` subprocess so `--run` measures the real SDK path, but it is CLI-internal. See the Decisions section before promoting it to a shipped runtime.
- **`zodToJsonSchema` handles zod v3 AND v4** and is tested against both real libraries (`zod3`/`zod4` devDependency aliases, `tests/sdk/zodVersions.test.ts`). The older mock-based tests in `zodToJsonSchema.test.ts` stay, but do not add v4 coverage there — mocks shaped like one version silently miss the other, which is exactly how v4 support was broken for months.
- **Template runtime adapters ARE typechecked, even though templates are not.** `packages/create-machine-app/tsconfig.adapters.json` checks all seven adapters against the real SDK contract via `paths` → `../../src/index.ts`; external deps are satisfied by ambient stubs in `packages/create-machine-app/types/` (`llama.rn`, `electron`, `@mediapipe/tasks-genai`, `@mlc-ai/web-llm`) — the same pattern `packages/ui` uses for `react-native`. Run via `npm run typecheck:adapters`, wired into the scaffolder's `check`. **If you add or edit a runtime adapter, it must pass this.** Three shipped adapters carried a broken `complete`/`completeChat` signature for months precisely because this didn't exist. **Extending a stub is a smell** — it means the adapter is reaching into backend internals instead of staying on the `ActivationRuntime` contract.
...
| **M6** | `machineai-activation-ui` headless UI kit (ModelPicker, CartridgeCard, ActivationStatus, InferenceIndicator, ModelImportButton) | Apps don't rebuild basic UX | M1 | 2 |
...
## M6 — `machineai-activation-ui` headless UI kit ✅ shipped (session 7, 2026-04-20)
...
- One package, two entry points: `machineai-activation-ui/web` and `machineai-activation-ui/native`. Same API, platform-specific implementations.
...
- `npm publish machineai-activation@0.2.0-beta.1`.
- `npm publish create-machine-app@0.1.0`.
- `npm publish machineai-activation-ui@0.1.0-beta.1`.
...
Next session: **M6 — `machineai-activation-ui` headless UI kit** (ahead of M5 so scaffolded templates can target the UI kit).
...
Next session: **M6 — `machineai-activation-ui` headless UI kit.** Execution order stays M6 → M5 → M8 → M9.

---

### Session 7 — 2026-04-20
**Milestone: M6 — `machineai-activation-ui` headless UI kit — ✅ shipped (first workspace split + 6 hooks + 5 components × {web, native} + 14 core unit tests)**
...
- ✅ `packages/ui/src/core/` — 8 files, all platform-neutral, import only from `react` + `machineai-activation`:
...
- ✅ `src/bin/commands/describe.ts` — new `ui` section in the payload (`machineai-activation-ui`, version, exports, hook list, component-per-target list). `M6` added to `shippedMilestones`; new pointer keys (`uiPackage` / `uiCore` / `uiWeb` / `uiNative`). Existing `describe` test still passes (its key-check list is exhaustive for stable keys only).
- ✅ `AGENTS.md` — file-pointer row for the UI workspace + a new recipe *"Add a new UI component to `machineai-activation-ui`"*, plus `npx machine describe ui` in the dump-the-surface block.
...
- **`machineai-activation` is NOT declared as a peer dep in `packages/ui/package.json` yet.** The root workspace *is* `machineai-activation` — listing it as a peer of a workspace child triggers E404 on `npm install` (npm tries to fetch the repo's own package from the registry). The UI kit still imports from `machineai-activation` via a `tsconfig.json#paths` alias, and the peer dep will be re-added at publish time (M9). Documented in `packages/ui/README.md`.
- **Split tsconfigs: src paths for dev, dist paths for build.** `tsconfig.json` aliases `machineai-activation` to `../../src/index.ts` so typecheck + tests run against live SDK source (matches SDK-repo workflow). `tsconfig.build.json` aliases to `../../dist/index.d.ts` so the published build doesn't violate the declaration-build's `rootDir` constraint. Trade-off: UI `build` now depends on the SDK `build` having run first; the root `check:all` script orders them correctly.
...
- **Templates not typechecked in this repo.** `templates/` is excluded from both `tsconfig.json` and `tsconfig.build.json` — the template files import `react-native`, `llama.rn`, `expo`, `machineai-activation-ui/native`, none of which resolve in the SDK workspace. Correctness comes from scaffolding into a temp dir + asserting file-tree shape + placeholder substitution in tests. Runtime verification (`npm install`, `expo start`) is a manual/downstream step. Trade-off accepted because the alternative — installing RN + Expo + Next.js in the SDK repo just to typecheck the template trees — would add hundreds of MB of devDeps for a one-line payoff.
...
- ✅ **`rn-cli-local-chat` template** — 11 files. Bare React Native 0.74 (no Expo) parallel to `expo-local-chat`: same `llamaRuntime.ts` (llama.rn adapter) + same `ChatScreen.tsx` (`machineai-activation-ui/native` hooks) copied verbatim, differs only in project shape. Files: `package.json.tmpl` (RN 0.74, `@react-native-community/cli`, `llama.rn@^0.5`, `machineai-activation`, `machineai-activation-ui`, `@react-native/metro-config` + `@react-native/babel-preset` devDeps, scripts `start`/`android`/`ios`/`tsc`), `tsconfig.json` (extends `@react-native/typescript-config`), `app.json.tmpl` (`{ name, displayName }` bare-RN convention), `babel.config.js` (`module:@react-native/babel-preset`), `metro.config.js` (`getDefaultConfig(__dirname)`), `.gitignore` (RN CLI flavor — `ios/Pods/`, `android/.gradle/` etc), `index.js.tmpl` (`AppRegistry.registerComponent('{{APP_NAME}}', () => App)`), `src/App.tsx` (`SafeAreaProvider` from `react-native-safe-area-context` wrapping `<MachineProvider machine={createMachine({ runtimes: llamaRuntime })}><ChatScreen /></MachineProvider>`), `src/ChatScreen.tsx`, `src/llamaRuntime.ts`, `README.md.tmpl` (RN CLI setup — `npx pod-install`, `{{PACKAGE_MANAGER}} run android`/`ios`).
- ✅ **`electron-local-chat` template** — 16 files, main/renderer process split with IPC-backed inference. Main process gets the real `node-llama-cpp` runtime; renderer uses a proxy `ActivationRuntime` that forwards through `window.machine.*` via `contextBridge`. Files:
  - `package.json.tmpl` (`electron@^33`, `electron-builder@^25`, `node-llama-cpp@^3`, `react@^18`, `react-dom@^18`, `vite@^5`, `@vitejs/plugin-react`, `concurrently`, `wait-on`, `machineai-activation`, `machineai-activation-ui`. `build.appId: "com.example.{{APP_NAME}}"` + `build.productName: "{{APP_NAME}}"` for electron-builder. Scripts: `dev` (concurrent vite + tsc watch + electron via wait-on), `build` (tsc + vite build + electron-builder), `start`).
  ...
  - `src/ipcRuntime.ts` — **renderer-side proxy ActivationRuntime.** Lets `machineai-activation-ui/web` hooks work unchanged: `createSession()` returns a session whose `complete`/`completeChat`/etc forward through `window.machine.complete(...)` IPC. Critical piece that makes the IPC split transparent to hooks.
  - `src/index.html.tmpl` (title `{{APP_NAME}}`), `src/main.tsx` (React DOM mount), `src/App.tsx` (MachineProvider + ChatScreen), `src/ChatScreen.tsx` (uses `machineai-activation-ui/web`), `.gitignore` (Electron flavor — `dist/`, `release/`, `out/`), `README.md.tmpl`.
- ✅ **`next-local-chat` template** — 10 files. Next.js 14 app-router web template with `@mlc-ai/web-llm` wired **inside the template** (not as a new SDK workspace). Files: `package.json.tmpl` (`next@^14`, `react@^18`, `@mlc-ai/web-llm@^0.2`, `machineai-activation`, `machineai-activation-ui`, scripts `dev`/`build`/`start`/`lint`/`tsc`), `tsconfig.json` (Next standard — `jsx: preserve`, `moduleResolution: bundler`), `next.config.js` (`transpilePackages: ['machineai-activation', 'machineai-activation-ui']` so workspace ESM resolves under Next's build), `next-env.d.ts`, `.gitignore` (Next flavor — `.next/`, `out/`), `src/app/layout.tsx` (root `<html><body>{children}</body></html>`), `src/app/page.tsx` (`'use client'` — wires `<MachineProvider machine={createMachine({ runtimes: webLlmRuntime })}><ChatScreen /></MachineProvider>`), `src/app/ChatScreen.tsx` (`'use client'` — `machineai-activation-ui/web` hooks), `src/lib/webLlmRuntime.ts` (`ActivationRuntime` wrapping `CreateMLCEngine` from `@mlc-ai/web-llm`, streams via `engine.chat.completions.create({ stream: true })`, default model `Llama-3.2-1B-Instruct-q4f32_1-MLC`), `README.md.tmpl` (notes WebLLM caches weights in browser IndexedDB, points at `@mlc-ai/web-llm` model catalog).
...
- ✅ **Tests — ~21 new, all green.** Three new files mirroring Session A's pattern:
  - `tests/rn-cli-local-chat.test.ts` — 6 tests (full tree, APP_NAME in `package.json` + `app.json`, `App.tsx` wires `MachineProvider` + `llamaRuntime`, `ChatScreen` uses `machineai-activation-ui/native`, `index.js` uses `AppRegistry.registerComponent`, no `.tmpl` leakage).
  - `tests/next-local-chat.test.ts` — 7 tests (tree, APP_NAME + deps assertions `next`/`@mlc-ai/web-llm`/`machineai-activation-ui`, `webLlmRuntime.ts` imports `@mlc-ai/web-llm` + uses `CreateMLCEngine`, `page.tsx` is `'use client'` + wires provider, `ChatScreen` uses `machineai-activation-ui/web`, `next.config.js` sets `transpilePackages`, no `.tmpl` leakage).
  - `tests/electron-local-chat.test.ts` — 8 tests (tree including `electron/` + `src/` subtrees, APP_NAME substitution across `package.json` + `build.appId` + `build.productName` + `index.html` title, `nodeLlamaRuntime.ts` imports `node-llama-cpp` + uses `getLlama`/`LlamaChatSession`, `main.ts` uses `ipcMain.handle` + registers `'machine:complete'`, `preload.ts` uses `contextBridge.exposeInMainWorld('machine', ...)`, `ChatScreen.tsx` uses `machineai-activation-ui/web`, `ipcRuntime.ts` forwards through `window.machine.complete`, no `.tmpl` leakage).
...
- ✅ **`package.json`:** `machineai-activation` dep repointed to `"file:../Machine AI/iterations/MachineActivationSDK"`; `zod@^3.23.0` added as a direct dep (peer-optional to the SDK but needed at the call site for `zodSchema(IngredientAnalysisSchema)`).
...
- ✅ **#1 — `machineai-activation-capacitor` workspace.** New `packages/activation-capacitor/` ships `machineai-activation-capacitor@0.1.0-alpha.1`. Mirrors the SDK root's dual-build pattern. Three source files (`runtime.ts`, `plugin.ts`, `filePicker.ts`) lifted from Reference App 01 verbatim; `runtime.ts` was lightly refactored to replace `this.listBackendCapabilities!()`/`probeDeviceCapabilities!()`/`probeModelPackage!(input)` non-null assertions with module-scope helper functions (the runtime contract changed `Reporters` to `Partial<Reporters>` between old + new SDK). Public API: `createCapacitorMachineActivationRuntime`, `registerCapacitorMachineActivationRuntime`, `pickMachineActivationModel`, `isMachineActivationPluginAvailable`, `MachineActivationNative`, plus type re-exports. Peer-deps: `@capacitor/core >=5.0.0`, `@capawesome/capacitor-file-picker >=6.0.0` (optional), `machineai-activation *` (workspace-resolvable).

- ✅ **#3 — Browser-safe main barrel (post-build-discovery).** First Ahara rebuild after #1+#2 still failed with `"readFile" is not exported by "__vite-browser-external", imported by "node_modules/machineai-activation/dist/esm/cartridge/nodeFs.js"`. Root cause: `src/cartridge/index.ts` and `src/catalog/index.ts` had `export { createNodeCartridgeFileSystem } from './nodeFs'` etc. — static ESM star-exports keep `node:fs/promises` / `node:path` / `node:crypto` imports in the consumer's bundle graph regardless of `sideEffects: false`. The legacy `package.json.browser` field map was tried first and ignored (modern bundlers prefer `exports` map over `browser` for path-to-path remapping). Real fix: created `src/node.ts` exporting all `createNode*` / `packCartridge` / `unpackCartridge` / `downloadAndUnpackCartridge` helpers, removed those names from `src/index.ts` + `cartridge/index.ts` + `catalog/index.ts`, and added `./node` to the `exports` map. CLI commands (`pack`, `unpack`, `validate`, `info`, `inspect`, `list`, `pull`) updated to deep-import the underlying files (e.g. `import { unpackCartridge } from '../../cartridge/nodeUnpackCartridge'`) since they always run in Node and don't need to go through the sub-export. `node-script` template updated to `import { createNodeCartridgeResolver } from 'machineai-activation/node'`.

- ✅ **Reference App 01 re-port.**
  - `src/main.tsx` — added `import { registerCapacitorMachineActivationRuntime } from 'machineai-activation-capacitor'` + the registration call. (This was a latent bug in Sessions 10/11 — the runtime was never being registered in the new app.)
  - `src/services/machineActivationService.ts` — switched plugin/probe imports from local `./machineActivationCapacitorPlugin` to `machineai-activation-capacitor`. File grew slightly (247 → 265 LOC, +18) because it now declares the consumer-facing `MachineActivationModelConfig` interface directly instead of leaning on the file-picker module's re-exports. The 300-LOC normalizer layer still gone, the `createMachineFramework(...).createActivationClient().activateModel(...)` ceremony still gone.
  - `src/components/SettingsView.tsx` — `pickMachineActivationModel` import switched from local `../services/machineActivationFilePicker` to `machineai-activation-capacitor`.
  - **Deleted:** `src/services/capacitorMachineActivationRuntime.ts` (~381 LOC), `machineActivationCapacitorPlugin.ts` (~146 LOC), `machineActivationFilePicker.ts` (~36 LOC). All three now provided by `machineai-activation-capacitor`.
  - **App `package.json`:** added `"machineai-activation-capacitor": "file:../Machine AI/iterations/MachineActivationSDK/packages/activation-capacitor"`. SDK dep already pointed at the workspace; no change needed there.
  - **`vite.config.ts`:** removed `optimizeDeps.include: ['machineai-activation']` + `build.commonjsOptions.include`. Added `resolve.preserveSymlinks: true` (only because we're still `file:`-linking the SDK + sub-package from a sibling monorepo; removable post-publish). Net effect: Vite config is back to "stock" + one symlink flag, no SDK-shaped escape hatches.
...
- **CLI commands deep-import instead of going through `machineai-activation/node`.** They always run in Node, never get bundled, and the deep paths (`../../cartridge/nodeUnpackCartridge`) match the existing relative-import style of the codebase. Going through the sub-export would have meant rewriting every CLI command's import to use the package name during build, which buys nothing.
- **Kept the test for `machineai-activation-capacitor` minimal.** `tests/run.ts` is a build-artifact existence check rather than a runtime-import test, because TS path aliases can't resolve `machineai-activation` at Node runtime against a file-linked workspace without `npm install` having run. The build check is sufficient — typecheck catches API surface mismatches, the host app's build catches integration mismatches.
...
**Why this session is M-shaped, not just polish:** The headline claim of the SDK is "plug a local model into your app cheaply." Sessions 10/11 proved that *given* a perfectly wired-up app — but the wiring itself was 1186 LOC of mostly-boilerplate adapter glue + a Vite config with two SDK-specific escape hatches. Until session 12, the per-app cost of *getting to* the clean call site dominated the per-app savings *at* the call site. After session 12, the consumer-side cost is `npm install machineai-activation machineai-activation-capacitor` + one `registerCapacitorMachineActivationRuntime()` line, and the savings stay (300-LOC normalizers + activation ceremony, gone). Reference App 01's Capacitor port is now genuinely small.
...
- **Did NOT build a model picker UI.** Cartridge id is hard-coded as `dev.machine.gemma-3n-e4b-it`. User swaps the constant in `electron/main.ts` + `src/components/ChatPanel.tsx` to point at their own model. Could be replaced with `<ModelPicker>` from `machineai-activation-ui/web` later.
...
- `scripts/prepackage.js` — removes the `node_modules/machineai-activation-*` symlinks before electron-builder packages (workspace `file:` deps create junctions pointing outside the app root, which electron-builder's asar packer rejects).
- esbuild used to bundle the main process into `dist/electron/main.js` so `machineai-activation` is inlined; the app no longer needs to ship `node_modules/machineai-activation-*` at runtime.
...
- Wrong import subpath: `MachineProvider` + `useMachineModel` were imported from `machineai-activation-ui/web` but are only exported from `machineai-activation-ui` root. In dev with `bundler` resolution this somehow worked; in production bundle they resolved to `undefined` → React error #130 (black screen). Now fixed throughout the template AND Reference App 02. Worth checking other templates use the right subpath too — `expo-local-chat`, `rn-cli-local-chat` correctly use `machineai-activation-ui/native`; `next-local-chat` still uses `machineai-activation-ui/web` (which IS correct there since it only renders visual components, not hooks).
...
**Product focus sharpened, then all runtime adapters brought onto the contract across five platforms.** Full ledger in `PLATFORM_MATRIX.md`; queue in `TODO.md`.

**Focus decision (drives everything below).** Machine AI is *the adapter layer for local models* — plug a GGUF into any app, on any OS, to **test** it and to **package** it. The Vercel-AI-SDK-shaped surface stays as the **porting on-ramp** but is explicitly no longer the identity: the cloud API shape has no vocabulary for model load time, RAM fit, thermal, model swapping, or quantization tradeoffs, and the activation contract *is* that vocabulary. Scope is Windows + macOS + Linux + Android + iOS.

**The two deferred items from session 14 were both bigger than "small follow-up."**

1. *"Extend `fetch-llama-cpp.js` for macOS arm64 + Linux x64"* (line 908) — done, plus the runtime side it implied. `HOST_TARGETS` maps `process.platform:process.arch` → release asset + vendor slug + binary name; POSIX extraction via `unzip` (falling back to `ditto`); executable bit restored; `LLAMA_CPP_ASSET` env override for CUDA/Vulkan builds; `version.json` now records asset/slug/exe/acceleration so `llamaServerRuntime.ts` resolves from record instead of guessing, and passes `--n-gpu-layers` when the vendored build supports it.

2. *"`schemaVersion: 1` should be `ACTIVATION_CONTRACT_SCHEMA_VERSION`"* (line 902) — was fixed only in the Electron template's own snapshot. The same numeric literal was still in `ipcRuntime.ts`, both mobile adapters, and the web adapter. All fixed.

**Three of seven adapters did not satisfy `ActivationSession`.** `expo-local-chat`, `rn-cli-local-chat`, and `next-local-chat` all declared `complete: async ({ prompt, system, maxTokens, stream, signal }) => …`, destructuring the **first argument** as an object while the contract passes `(prompt, options)` / `(messages, options)`. Every call from `generateText`/`streamText` passed a string where an object was expected. They also referenced a `stream` option that doesn't exist on `ActivationCompletionOptions`. This is the identical defect session 13 fixed in the Electron template — it was never back-ported to the other three, and nothing caught it because templates aren't typechecked.

**Grammar was dead on mobile.** Both mobile adapters hardcoded `structuredJsonOutput: false` / `toolCalling: false` and never forwarded `options.grammar` — so the portable JSON-Schema → GBNF emitter shipped in M7, the SDK's most differentiated feature, was switched off on the platform that needs it most. Both now call `resolveStructuredOutputGrammar(...)` (which also maps `responseFormat: 'json'`). **GPU offload was likewise disabled** (zero layers); they now request a full offload and report *observed* acceleration via `context.gpu` / `reasonNoGPU` / `devices`, surfacing the reason as a contract warning when offload fails.

**`completeChat` discarded conversation history** in `llamaServerRuntime` — it collapsed to `messages[messages.length - 1]`, breaking multi-turn chat and silently breaking the tool loop (which works by appending `assistant` + `tool` turns). Now forwards full history. All four llama/web adapters also fold the `tool` role into a user turn (`sdkgaps.md` #5) since Gemma/Llama-3/ChatML templates have no such role.

**Guardrail — the actual fix.** Every defect above shares one root cause, and it's already stated in this file: *"Templates aren't typechecked in CI so the mismatch never surfaced."* `packages/create-machine-app/tsconfig.adapters.json` now typechecks all seven adapters against the real contract (`paths` → `../../src/index.ts`), with ambient stubs in `packages/create-machine-app/types/` for `llama.rn`, `electron`, `@mediapipe/tasks-genai`, `@mlc-ai/web-llm` — the same pattern `packages/ui` uses for `react-native`. Wired into the scaffolder's `check`, so `check:all` covers it. **It caught the `ipcRuntime` `schemaVersion` bug on its first run.** Templates as a whole stay un-typechecked; only the adapters, which implement an SDK-defined contract, are covered.

Scaffolder tests: **44 → 52**, all green; 8 new tests pin each fix (including negative assertions so the destructured signature can't come back).

**Tension worth flagging for a future session:** the 2026-04-19 decision *"Tool-loop grammar is all-or-nothing"* (see Decisions below) is now contradicted by field experience — `sdkgaps.md` #2, written from the Collecta integration, argues the envelope should stay grammar-locked even when a tool lacks a schema, because small local models drift into prose without it. The original rationale (strict back-compat for incremental Zod adoption) is real but looks like the weaker trade now. That decision should be revisited explicitly, not silently reversed.

**Not done: none of this has run on real hardware.** Every non-Windows row in `PLATFORM_MATRIX.md` is "wired and typechecked," not "verified." Typechecking proves shape, not behavior — running one GGUF on a Mac, a Linux box, an iPhone, and an Android device is now the highest-value open item (`TODO.md` item 2).

**Files touched:** `packages/create-machine-app/templates/{electron-local-chat/{scripts/fetch-llama-cpp.js,electron/llamaServerRuntime.ts,src/ipcRuntime.ts},rn-cli-local-chat/src/llamaRuntime.ts,expo-local-chat/src/llamaRuntime.ts,next-local-chat/src/lib/webLlmRuntime.ts}`, `packages/create-machine-app/{tsconfig.adapters.json,package.json,types/*.d.ts}`, `packages/create-machine-app/tests/{electron,expo,rn-cli,next}-local-chat.test.ts`, plus new `PLATFORM_MATRIX.md` and a rewritten `TODO.md`. **No `src/` changes** — the SDK core was not modified.

### Session 16 — 2026-07-24
**A model actually ran.** Windows x64 moved from "wired" to **verified**, `machine doctor` shipped, and the five open SDK defects in the `TODO.md` queue were closed. Ledger in `PLATFORM_MATRIX.md`; queue in `TODO.md`.

**First inference in the project's history.** `Qwen2.5-0.5B-Instruct-Q4_K_M.gguf` (379 MB) loaded through a vendored `llama-server.exe` on an i7-10510U / 16 GB / Windows 11 box: **3.7 s load, 1.05 s to first token, 12.3 tok/s decode**, and grammar-constrained `generateObject` returned `{"language":"French","confidence":0.95}`. A separate live harness then exercised every SDK change from this session against the same model — streaming deltas, the grammar-locked tool loop, a schemaless tool, forced `toolChoice`, and `abortSignal` cancelling in 731 ms — 5/5 passing.

Running it immediately surfaced two things static analysis had not: throughput was being computed across prompt-evaluation time (making a short generation look ~5× slower than the model decodes), and a bad binary path escaped as an uncaught `spawn` ENOENT instead of an error message. Both fixed. That is the argument for item 2 in one paragraph.

**`machine doctor <model.gguf> [--run]` — the *test* verb, finally exposed.** Static mode needs no catalog, network, or runtime: a new portable GGUF header parser (`src/model/gguf.ts`, no `node:*`; file reads in `nodeGguf.ts`) pulls architecture, quantization, parameter count, context window and chat-template presence straight out of the file, and the report is assembled through `resolveCapabilityContract` so the memory verdict comes from the activation contract rather than a parallel implementation. `--run` loads the model via `llama-server` and reports observed load time, TTFT, decode rate, acceleration, and whether grammar-constrained JSON *actually* works — by calling the real `generateObject`, not a bespoke probe. Wired into `machine describe` for agents.

**The "CLI pull/search localhost flake" was never a flake.** Making the three test harnesses sequential (with a per-test timeout and a slow-test duration marker) fingered it in one run: `machine pull exits 1 when the catalog does not contain the id` took **309,684 ms**. It used the blocking `runCli` (`spawnSync`) while needing the in-process catalog server to answer — the parent event loop was held for the child's whole life, the server never responded, and the child sat until undici's 300 s headers timeout and exited 1. The test asserts exit code 1, so it *passed*, for the wrong reason, burning five minutes every run. Every other server-backed test already used `runCliAsync`. One-line fix, plus an assertion that the error mentions the missing id so it can't pass on a timeout again, plus a 30 s `spawnSync` timeout in `runCli` (a blocked loop means the harness's own timer can't fire). **Suite: ~4–5 min → 37 s.**

**Four SDK defects closed.** (1) The tool-loop **grammar cliff** — the envelope is now always locked, only an unschema'd tool's `args` degrades; this reverses the 2026-04-19 all-or-nothing decision explicitly, per the tension session 15 flagged. (2) **`abortSignal`** is now on `ActivationCompletionOptions` and honored by `generateText`/`streamText`/`generateObject`, with listener cleanup so a settled call can't cancel a later one on the same session. (3) **`toolChoice`** implemented — `'none'` skips the loop, `{ toolName }` forces that tool on step 0 only (grammar drops the `answer` branch and all other tools), reverting to `auto` so the loop can still terminate. (4) The caller's **`system` prompt is no longer sent twice**.

**Zod v4 support, and a lesson about mock-shaped tests.** The peer range was capped below v4, and consumers who installed it anyway with `--legacy-peer-deps` got a walker that silently returned `null` — dropping the grammar rather than failing — because v4 renamed `_def.typeName` → `_def.type`, moved array elements to `_def.element`, literals to `_def.values`, enums to `_def.entries`, and encodes int as `format: 'safeint'`. **No v3-shaped mock could ever have caught that.** The walker now handles both versions, `zod3`/`zod4` are installed side by side as devDependency aliases and round-trip tested against the real library, and the peer range is `>=3.22.0 <5`. The duck-typing rule (no `import 'zod'` in `src/sdk/**`) is unchanged.
...
- **2026-07-24** — **The product is the adapter layer for local models, and the AI-SDK-shaped surface is the porting on-ramp, not the identity.** Two verbs: **test** (does this model run *here*, does it fit, how fast, what's degraded) and **package** (ship the model inside your app). Scope is Windows + macOS + Linux + Android + iOS. Rationale: the cloud API shape has no vocabulary for model load time, RAM fit, thermal, model swapping, or quantization tradeoffs — there is no `model.load()` in a cloud SDK because loading is free. The activation contract *is* that vocabulary and was buried under a borrowed surface. `generateText`/`streamText`/`generateObject` stay (they make porting a one-import change, which nothing beats) but they no longer lead the docs. Consequence: invest in the activation contract, the `.mcart` cartridge format, and the catalog; treat the shim as a compat layer.
- **2026-07-24** — **Runtime adapters are typechecked against the contract even though templates are not.** Templates reference deps that only exist post-scaffold, so typechecking them wholesale isn't practical — but adapters implement an SDK-*defined* contract, and drift there is invisible until someone scaffolds and runs. Three of seven shipped adapters had a broken `complete`/`completeChat` signature for months because of this gap. `tsconfig.adapters.json` + ambient stubs in `packages/create-machine-app/types/` close it, reusing the `packages/ui` react-native-stub pattern. **Extending a stub is a smell** — it means an adapter is reaching into backend internals instead of staying on the contract.
- ~~**2026-07-24 (open tension)**~~ — **resolved 2026-07-24 (session 16): the 2026-04-19 "tool-loop grammar is all-or-nothing" decision is reversed.** The envelope (`{"tool","args"} | {"answer"}`) is now *always* grammar-locked; only an individual tool's `args` degrades — to "any JSON object" — when its `parameters` can't self-describe. The original decision protected incremental Zod adoption, but it traded the wrong thing: the schemaless tool's *args* were never the reliability risk, the unconstrained envelope was. Verified on a 0.5B model, which called a schemaless tool correctly and terminated in two steps under the locked envelope.
- **2026-07-24 (session 16)** — **`abortSignal` belongs on the adapter contract, not just in SDK bookkeeping.** `ActivationCompletionOptions.abortSignal` is optional and adapters may ignore it (the SDK also calls `session.abort()`), but honoring it is strictly better because it scopes cancellation to one in-flight completion instead of the whole session. Landing it now, while there are seven first-party adapters and no third-party ones, is much cheaper than landing it later. Abort rejections are plain `Error` with `name === 'AbortError'` — **not** `DOMException`, which isn't reliably present across React Native runtimes.
- ~~**2026-07-24 (session 16)**~~ — **reversed 2026-07-25 (session 17): the llama-server runtime is public, and lives in the SDK core.** The original entry said "promote it only if a concrete consumer asks." Four asked at once: `Collecta-Local` and `Ingredient analyzer` had each hand-written their own, the `electron-local-chat` template vendored a third, and `doctor` held the fourth. They had all diverged — dropped chat history in one, `streaming: false` in another, a `capabilitySnapshot` missing `schemaVersion` in a third — and no fix ever propagated between them. The feared cost (maintaining a public surface across llama.cpp releases) was already being paid four times over, without the benefit of a single tested implementation. Now `src/runtime/`: `llamaServerRuntime` + `stubRuntime` portable from the main barrel, `startLlamaServer`/`ensureLlamaServer` from `machineai-activation/node`. **Core, not a satellite package** — the adapter has zero dependencies, and a separate package forces a circular build with the SDK's own `doctor`, which needs it. "Bring your own runtime" is unchanged: `ActivationRuntime` is still a three-member interface and any backend can implement it. One of them now ships in the box.
- **2026-07-25 (session 17)** — **`machine serve` exists because the SDK being TypeScript is a hard ceiling, not a preference.** `Agent On Deck - Local`'s entire AI surface is Python (`sidecar/gemini.py`, 13 call sites); no amount of SDK polish would have let it plug in. Serving the model over an OpenAI-shaped HTTP surface makes the language question disappear — the wire format becomes the contract, and Python/Go/Ruby/Swift all reach it. Deliberately **not** a passthrough proxy to `llama-server`: it routes through `createMachine`, so `response_format.json_schema` compiles to GBNF via the SDK's own emitter, cartridge ids resolve from the local cache, and `/machine/activation` exposes the contract vocabulary — none of which a bare llama-server offers. `/machine/activation` is intentionally not OpenAI-shaped; load time, memory fit and degradation have no place in a schema built for an API where the model is always loaded.
- **2026-07-25 (session 17)** — **`extraBody` on the llama adapter, so one missing knob is not a reason to fork.** Collecta needed `min_p` and `repeat_penalty`, which the activation contract does not model — and *"the contract lacks one field I need"* is precisely how each duplicated adapter in this workspace began. `extraBody` fills only fields a call left unset (spreading it either way is wrong: spread-first and an unset `temperature: undefined` deletes the default, spread-last and an explicit caller value is ignored), and `messages`/`stream`/`grammar`/`model` are reserved so a config knob can never silently break structured output.
- **2026-07-24 (session 16)** — **Backend capability and backend availability are different questions.** Doctor's first cut set `sessionCreationAvailable: false` when no `llama-server` binary was found, which made `resolveCapabilityContract` declare a perfectly healthy model "not recommended on this device". The contract answers "what would this model+backend combination support"; whether a binary happens to be installed is an advisory for the human. Conflating them makes the contract lie.
- **2026-07-24 (session 16)** — **Zod support stays duck-typed, but now spans v3 *and* v4, tested against the real library.** The 2026-04-19 "no `import 'zod'` in `src/sdk/**`" rule holds — the walker still reads `_def` and nothing else. What changed is that mock-shaped tests were proven insufficient: v4 renamed `_def.typeName` → `_def.type`, and the walker silently returned `null` (dropping the grammar) rather than failing, so no v3-shaped mock could ever have caught it. `zod3` and `zod4` are now installed side by side as devDependency aliases and round-trip tested. Peer range widened to `>=3.22.0 <5`.
