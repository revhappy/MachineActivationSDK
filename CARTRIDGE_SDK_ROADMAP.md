...
- **Tests:** root `npm run check` — typecheck + build green; SDK test suites green. UI kit has its own `npm run check:ui` / `npm run check:all` at the root (typecheck + 14 core unit tests + web+native TS build). `machineai-activation-capacitor` (sub-package, session 12) has its own `check` script that typechecks + smoke-tests the build artifacts. Pre-existing CLI pull flake note below.
...
- **Agent entry point:** `AGENTS.md` + `machine describe` (dumps the full SDK surface as JSON in one call; now includes a `ui` section listing `machineai-activation-ui` hooks + components and a `scaffolder` section listing `create-machineai-app` templates).
...
- **Publish to npm.** All four workspaces (`machineai-activation`, `machineai-activation-ui`, `machineai-activation-capacitor`, `create-machineai-app`) need version bumps + `npm publish --access public`. Drop the `file:` deps in the reference apps and replace with version ranges.
