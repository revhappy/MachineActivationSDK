...
- **2026-07-24 (session 16)** — **`machine doctor`'s live-run runtime stays CLI-internal.** `src/bin/commands/doctorRuntime.ts` is a real `ActivationRuntime` over a `llama-server` subprocess, but it is *not* exported from the SDK. Shipping a public Node runtime is a commitment to an API surface we'd have to maintain across llama.cpp releases, and the `node-script` template's "bring your own runtime" stance is deliberate. Doctor needs it to make `--run` measure the *real* SDK path (`createMachine` → `streamText`/`generateObject`, grammar included) rather than a bespoke HTTP probe. Promote it only if a concrete consumer asks.
