...
A directed reading order for `@machine/activation-sdk`. Each link below is a markdown file at the repo root or inside this directory.
...
- `src/node.ts` — node-only sub-export (`@machine/activation-sdk/node`) for tooling.

## Other workspaces in this repo

| Path | Package | Role |
|---|---|---|
| `packages/ui/` | `@machine/ui` | Headless React + RN UI kit (hooks + 5 components per target). |
| `packages/create-machine-app/` | `@machine/create-machine-app` | Scaffolder with 5 templates (expo, rn-cli, next, electron, node). |
| `packages/activation-capacitor/` | `@machine/activation-capacitor` | Capacitor (Android) runtime adapter bridging LiteRT-LM. |
