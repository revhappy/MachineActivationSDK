...
Sections (omit for full payload):
  ...
  ui            @machine/ui headless React / React Native UI kit.
  scaffolder    @machine/create-machine-app CLI + available templates.
...
const UI_PACKAGE: UiPackageDescription = {
  ...
  exports: ['@machine/ui', '@machine/ui/web', '@machine/ui/native'],
  hooks: [
    {
...
const SCAFFOLDER_PACKAGE: ScaffolderPackageDescription = {
  name: '@machine/create-machine-app',
  version: '0.1.0-alpha.1',
  ...
  templates: [
    ...
    },
    {
      ...
      description: 'Electron desktop app with streaming on-device chat via node-llama-cpp (main) + @machine/ui/web (renderer).',
      target: 'electron',
    },
  ],
};
